...
struct Selective_Scan2_bwd_kernel_traits {
...
void selective_scan2_bwd_kernel(SSMParamsBwd params) {
...
void selective_scan2_bwd_launch(SSMParamsBwd &params, cudaStream_t stream) {
    BOOL_SWITCH(params.seqlen % (kNThreads * kNItems) == 0, kIsEvenLen, [&] {
        BOOL_SWITCH(params.is_variable_B, kIsVariableB, [&] {
            BOOL_SWITCH(params.is_variable_C, kIsVariableC, [&] {
                BOOL_SWITCH(params.delta_softplus, kDeltaSoftplus, [&] {
                    BOOL_SWITCH(params.z_ptr != nullptr, kHasZ, [&] {
                        using Ktraits = Selective_Scan2_bwd_kernel_traits<kNThreads, kNItems, kIsEvenLen, kIsVariableB, kIsVariableC, kDeltaSoftplus, kHasZ, input_t, weight_t>;
                        constexpr int kSmemSize = Ktraits::kSmemSize + 2 * MAX_DSTATE * sizeof(weight_t) + kNThreads * sizeof(weight_t) + kNThreads * sizeof(weight_t) + MAX_DSTATE * sizeof(typename Ktraits::scan_t) + MAX_DSTATE * sizeof(weight_t) + MAX_DSTATE * sizeof(weight_t);
                        auto kernel = &selective_scan2_bwd_kernel<Ktraits>;
                        if (kSmemSize >= 48 * 1024) {
                            ...
                            C10_CUDA_CHECK(cudaFuncSetAttribute(
                                ...
                                std::cerr << "Warning (selective_scan2_bwd_kernel): attempting to set maxDynamicSharedMemorySize on an AMD GPU which is currently a non-op (in ROCm versions <= 6.1). This might lead to undefined behavior. \n" << std::endl;
                            #endif
                        }
...
void selective_scan2_bwd_cuda(SSMParamsBwd &params, cudaStream_t stream) {
    #ifndef USE_ROCM
        if (params.seqlen <= 128) {           
            selective_scan2_bwd_launch<32, 4, input_t, weight_t>(params, stream);
        } else if (params.seqlen <= 256) {
            selective_scan2_bwd_launch<32, 8, input_t, weight_t>(params, stream);
        } else if (params.seqlen <= 512) {
            selective_scan2_bwd_launch<32, 16, input_t, weight_t>(params, stream);
        } else if (params.seqlen <= 1024) {
            selective_scan2_bwd_launch<64, 16, input_t, weight_t>(params, stream);
        } else {
            selective_scan2_bwd_launch<128, 16, input_t, weight_t>(params, stream);
        }
    #else 
        if (params.seqlen <= 256) {
            selective_scan2_bwd_launch<64, 4, input_t, weight_t>(params, stream);
        } else if (params.seqlen <= 512) {
            selective_scan2_bwd_launch<64, 8, input_t, weight_t>(params, stream);
        } else if (params.seqlen <= 1024) {
            selective_scan2_bwd_launch<64, 16, input_t, weight_t>(params, stream);
        } else {
            selective_scan2_bwd_launch<128, 16, input_t, weight_t>(params, stream);
